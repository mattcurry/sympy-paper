...
namespace.\footnote{\texttt{import *} has been used here to aid the
  ...
  standard Python \texttt{math} library, and the popular numeric library
  NumPy~\cite{van2011numpy} all define the \texttt{exp} function, but only the
  SymPy one will work with SymPy symbolic expressions.} From here on, all
examples in this paper assume that this statement has been executed. All
examples in this paper use SymPy version 1.0, and should work with Python
versions 2.7, 3.2, 3.3, 3.4, or 3.5 in any operating system that supports
Python.
...
hashed and used as keys in Python dictionaries, thereby permitting features
such as caching.
...
The \texttt{dotprint} function from the \texttt{sympy.printing.dot} package
...
\texttt{Function}. All SymPy classes used for expression trees should be subclasses of the base class
\texttt{Basic}\footnote{Some
  internal classes, such as those used in the polynomial module, do not follow
  this rule for efficiency reasons.}, which defines some basic methods for symbolic expression
