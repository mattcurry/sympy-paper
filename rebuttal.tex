...
% TODO: Note that old assumptions do not use sympy.logic
% TODO: Reference the RETE algorithm
\item The ``Logic'' section is alluded to in the ``Assumptions'' subsection, but is
...
``old assumptions'', and do not use the \texttt{sympy.logic} submodule discussed in the
supplement. The so-called ``new assumptions'', mentioned briefly in the
...
\item Regarding the ``Diophantine Solvers'' section, the ``Solvers'' subsection
  already mentions that SymPy can solve Diophantine Equations. We consider
  this specific application to be niche enough to keep the discussion in the
  supplement. We have added a reference to the supplement to the ``Solvers''
  subsection.

\item The ``Statistics'' module is an independent
