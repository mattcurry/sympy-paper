...
\begin{solution}
  Both Python standard library and SymPy have exp callable, so it depends
  which symbols are imported in your global namespace: the \texttt{exp}
  function from the \texttt{math} module or the \texttt{exp} class from SymPy.
...
% TODO For projects that use SymPy
% TODO: Make intro sentence longer.
% TODO: Make it into a table
% TODO: Project name, domain, description
...
We deem all the sections in section 3 (now section 2) to be important core
components of the library. Regarding the supplement sections (previously) 5,
7, and 9---``Sets'', ``Category Theory'', and ``SymPy Live'',
respectively---only set sets module is potentially applicable to the wider
audience of the main paper. However, in SymPy 1.0, the sets module is still in
...
subsection regarding \texttt{sympy.sets}. The category theory module in SymPy
is very domain specific, and independent of other SymPy modules. We consider
the module too niche to warrant expanding upon in the main manuscript. SymPy
Live, as has been discussed above, is not a SymPy submodule. It is a separate
project from the SymPy library. It would be inappropriate to list it alongside
