...
Table~\ref{features-table} are symbolic in nature. The submodules in
parentheses are the \texttt{sympy} submodules where the features are
implemented (like \texttt{sympy.core}). Numeric features are discussed in
...
\midrule
Calculus (\texttt{core}, \texttt{series}, \texttt{integrals}) & Algorithms for computing derivatives, integrals, and limits.\\

Category Theory (\texttt{categories}) & Representation of objects, morphisms, and diagrams. Tools
for drawing diagrams with Xy-pic.\\

Code Generation (\texttt{printing}, \texttt{codegen}) & Generation of compilable and executable code in a
...
Combinatorics \& Group Theory (\texttt{combinatorics}) & Permutations, combinations,
...
Concrete Math (\texttt{concrete}) & Summation, products, tools for determining whether summation
...
Cryptography (\texttt{crypto}) & Block and stream ciphers, including shift, Affine,
substitution, Vigen\`{e}re's, Hill's, bifid, RSA, Kid RSA,
linear-feedback shift registers, and Elgamal encryption.\\

Differential Geometry (\texttt{diffgeom}) & Representations of manifolds, metrics, tensor
...
Geometry (\texttt{geometry}) & Representations of 2D geometrical entities, such as lines and
...
Lie Algebras (\texttt{liealgebras}) & Representations of Lie algebras and root systems.\\

Logic (\texttt{logic}) & Boolean expressions, equivalence testing, satisfiability, and normal
forms.\\

Matrices (\texttt{matrices}) & Tools for creating matrices of symbols and expressions.
Both sparse and dense representations, as well as symbolic linear
algebraic operations (e.g., inversion and factorization), are
supported.\\

Matrix Expressions (\texttt{matrices.expressions}) & Matrices with symbolic dimensions (unspecified entries).
Block matrices.\\

Number Theory (\texttt{ntheory}) & Prime number generation, primality testing, integer
...
Plotting (\texttt{plotting}) & Hooks for visualizing expressions via matplotlib~\cite{Hunter:2007}
...
Polynomials (\texttt{polys}) & Polynomial algebras over various coefficient domains.
...
Printing (\texttt{printing}) & Functions for printing SymPy expressions in the terminal with ASCII
or Unicode characters and converting SymPy expressions to \LaTeX{} and
MathML.\\

Quantum Mechanics (\texttt{physics.quantum}) & Quantum states, bra--ket notation, operators, basis sets,
...
Series (\texttt{series}) & Series expansion, sequences, and limits of sequences.
This includes Taylor, Laurent, and Puiseux series as well as special series, such
as Fourier and formal power series.\\

Sets (\texttt{sets}) & Representations of empty, finite, and infinite sets (including
...
Simplification (\texttt{simplify}) & Functions for manipulating and simplifying expressions.
...
Solvers (\texttt{solvers}) & Functions for symbolically solving equations, systems
...
Special Functions (\texttt{functions}) & Implementations of a number of well known special functions,
...
Statistics (\texttt{stats}) & Support for a random variable type as well as the ability to
...
Tensors (\texttt{tensor}) & Symbolic manipulation of indexed objects.\\

Vectors (\texttt{vector}) & Basic operations on vectors and differential calculus with respect
